...
const keyHint = (_keybinding: string, description: string) => description;

// This file is loaded outside pi's package scope, so runtime helpers live here.
class TodoBorder {
...
function getTodoMarkdownTheme(theme: Theme): MarkdownTheme {
  return {
...
async function copyToClipboard(text: string): Promise<void> {
  ...
  await new Promise<void>((resolve, reject) => {
    const child = spawn(command, args, { stdio: ["pipe", "ignore", "pipe"], shell: process.platform === "win32" });
    let stderr = "";
    child.stderr.on("data", (chunk) => { stderr += chunk; });
    child.on("error", reject);
    child.on("close", (code) => code === 0 ? resolve() : reject(new Error(stderr || `Clipboard command exited with ${code}`)));
...
class TodoSelectorComponent extends Container implements Focusable {
  ...
  constructor(
    ...
    this.addChild(new TodoBorder((s: string) => theme.fg("accent", s)));
...
class TodoDetailOverlayComponent {
  ...
  constructor(
    ...
    this.markdown = new Markdown(this.getMarkdownText(), 1, 0, getTodoMarkdownTheme(this.theme));
  }
    ...
    handler: async (args, ctx) => {
      ...
      await ctx.ui.custom<void>((tui, theme, _kb, done) => {
        ...
        const applyTodoAction = async (
          ...
          if (action === "copyPath") {
            await copyTodoPathToClipboard(record.id);
            return "stay";
          }
          if (action === "copyText") {
            await copyTodoTextToClipboard(record);
            return "stay";
          }
